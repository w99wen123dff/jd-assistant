...
//*************************公共方法和变量*************************
    ...
    function goToLogin() {
        if (isLocBuy()) {
            window.location.href = OrderAppConfig.LoginLocUrl + appendParamFlag();
        } else {
            window.location.href = OrderAppConfig.LoginUrl + appendParamFlag();
        }
    ...
    function consigneeList(selectId) {
      ...
      var param = "consigneeParam.newId=" + consigneeId;
          param = param + "&consigneeParam.addType=";
          param = param + addType;
      // param = addFlowTypeParam(param);;
      var userSmark = $("#userSmark").val();
      if(userSmark){
          param = param + "&consigneeParam.userSmark=" + userSmark;
      }
      param = addOverseaMergeParam(param)
      jQuery.ajax({
        type: "POST",
        dataType: "text",
        url: actionUrl,
    ...
    function consigneeChangeTip(){
        if($("#downConsigneeChangeTip").val() != 1){
            ...
            if($("#showChangeJingAddressFlag").val() =="true"){
            	var changeJingAddresCopy = $("#changeJingAddresCopy").val();
                 ...
                 if(changeJingAddresCopy){
                	 sj_li = sj_li+"<i></i>"+changeJingAddresCopy+"</div></div></li>";
                 }else{
      ...
      jQuery.ajax({
        ...
        success: function(consigneeResult, textStatus) {
          ...
          if (consigneeResult.success) {
              $("#addressBackCheckMessage").removeClass("message");
              $("#addressBackCheckMessage").html("");
              if($("#downDyb").val() == "1") {
                  window.asyncCurrentOrderResultView = consigneeResult.asyncCurrentOrderResultView;
                  $("#isNeed").val(consigneeResult.needPayPwd);
              }
            ...
            if($('#allHaoKa').val()=="true"){
                $("#part-inv").html("<span class='mr10'>" + $("#allHaoKaTip").val() +"&nbsp;</span>");
                    ...
                    if(consigneeResult.resetQqg){
                        $("#part-inv").html("<span class='mr10'>"+$('#qqgInvoice').val()+"&nbsp;</span><a onclick='edit_Invoice()' id='invoiceEdit' class='ftx-05 invoice-edit hide' href='#none'>修改</a>");
                    }
                    if(consigneeResult.resetCommon){
                        ...
                        } else if(consigneeResult.selectInvoiceType==22){
                            //电子增值税发票
                            $("#part-inv").html("<span class='mr10'>电子增值税发票&nbsp; </span><span class='mr10'> "+consigneeResult.selectInvoiceTitle+"&nbsp; </span><span class='mr10'>"+consigneeResult.selectNormalInvoiceContent+"&nbsp; </span><a onclick='edit_Invoice()' class='ftx-05 invoice-edit' href='#none'>修改</a>");
                        }
                    }
                }
    
            var consignee_provinceId = consigneeResult.consigneeShowView.provinceId;
            if(consignee_provinceId == hongKongId || consignee_provinceId == taiWanId || consignee_provinceId == overSeasId || $("#isInternational").val() == 'true'){
    ...
    //	      var payId = $('.payment-item.item-selected').attr('payId');
            ...
            if(payId == null || payId == "" || payId == "undefined" || payId == undefined || payId == "null"){
              ...
              pickParam = addOverseaMergeParam(pickParam);
    ...
    function delete_Consignee(id) {
      ...
      var commonConsigeeSize = $("#hidden_consignees_size").val();
      var consigneeSize = parseInt(commonConsigeeSize);
      var param = "consigneeParam.newId=" + id;
    ...
    function setAllDefaultAddress(id) {
        //抓取页面信息li
        var consigneeInput = $("#hid_pcArea_"+id);
        // 获取信息
        var consignee_id = consigneeInput.attr("consigneeId");
        var consignee_name = consigneeInput.attr("name") == "null"?"":consigneeInput.attr("name");
        var consignee_provinceId = consigneeInput.attr("provinceId") == "null"?"":consigneeInput.attr("provinceId");
        var consignee_cityId = consigneeInput.attr("cityId") == "null"?"":consigneeInput.attr("cityId");
        var consignee_countyId = consigneeInput.attr("countyId") == "null"?"":consigneeInput.attr("countyId");
        var consignee_townId = consigneeInput.attr("townId") == "null"?"":consigneeInput.attr("townId");
        var consignee_address = consigneeInput.attr("address") == "null"?"":consigneeInput.attr("address");
        var consignee_mobile = consigneeInput.attr("mobile") == "null"?"":consigneeInput.attr("mobile");
        var consignee_email = consigneeInput.attr("email") == "null"?"":consigneeInput.attr("email");
        var consignee_phone = consigneeInput.attr("phone") == "null"?"":consigneeInput.attr("phone");
        var consignee_provinceName = consigneeInput.attr("provinceName") == "null"?"":consigneeInput.attr("provinceName");
        var consignee_cityName = consigneeInput.attr("cityName") == "null"?"":consigneeInput.attr("cityName");
        var consignee_countyName = consigneeInput.attr("countyName") == "null"?"":consigneeInput.attr("countyName");
        var consignee_townName = consigneeInput.attr("townName") == "null"?"":consigneeInput.attr("townName");
        var giftSender_consignee_name = consigneeInput.attr("giftSenderConsigneeName") == "null"?"":consigneeInput.attr("giftSenderConsigneeName");
        var giftSender_consignee_mobile = consigneeInput.attr("giftSendeConsigneeMobile") == "null"?"":consigneeInput.attr("giftSendeConsigneeMobile");
        var consignee_idCard   = consigneeInput.attr("idCard") == "null"?"":consigneeInput.attr("idCard");
        var addressName = consigneeInput.attr("addressName") == "null"?"":consigneeInput.attr("addressName");
        var consignee_areaCode =  consigneeInput.attr("areaCode") == "null"?"":consigneeInput.attr("areaCode");
        var consignee_nameCode = consigneeInput.attr("nameCode") == "null"?"":consigneeInput.attr("nameCode");
        var consignee_postcode = consigneeInput.attr("postcode") == "null"?"":consigneeInput.attr("postcode");
        var consignee_longitude = consigneeInput.attr("longitude") == "null"?"":consigneeInput.attr("longitude");
        var consignee_latitude = consigneeInput.attr("latitude") == "null"?"":consigneeInput.attr("latitude");
        var consignee_coord_type = consigneeInput.attr("coord_type") == "null"?"":consigneeInput.attr("coord_type");

        var param = "consigneeParam.newId=" + consignee_id
            //+ "&consigneeParam.type=" + consignee_type
            + "&consigneeParam.name=" + consignee_name.replace(new RegExp("&","gm"),"%26")
            ...
            + "&consigneeParam.address=" + consignee_address.replace(new RegExp("&","gm"),"%26")
            ...
            + "&consigneeParam.idCard="+consignee_idCard
            + "&consigneeParam.addressName="+addressName.replace(new RegExp("&","gm"),"%26")
            + "&consigneeParam.areaCode=" + consignee_areaCode
            + "&consigneeParam.nameCode=" + consignee_nameCode
            + "&consigneeParam.zip=" + consignee_postcode;
        + "&consigneeParam.longitude=" + consignee_longitude;
        + "&consigneeParam.latitude=" + consignee_latitude;
        + "&consigneeParam.coord_type=" + consignee_coord_type;
        param = addFlowTypeParam(param);
        param = addOverseaMergeParam(param);
    ...
    function checkMaxConsigneeSize() {
      ...
      var commonConsigeeSize = $("#hidden_consignees_size").val();
      var limitAddressNum = $("#limitAddressNum").val();
      var addressNum;
      if(limitAddressNum){
          addressNum = limitAddressNum;
      }else{
          addressNum = 20;
      }
      if (commonConsigeeSize >= toNumber(addressNum))
      ...
      jQuery.ajax({
        ...
        success : function(dataResult, textStatus) {
          ...
          if (isHasMessage(dataResult)) {
            ...
            //留言初始化
            if($("#downDyb").val() == "1" && isPayTab != 1 && $("#flowType").val() != "15") {
                loadOrderExtNew(vendorMessage);
            } else if ($("#downDybZffs").val() == "1" && isPayTab == 1 && $("#flowType").val() != "15") {
                loadOrderExtNew(vendorMessage);
            } else {
    ...
    function flushPlusInfo(type){
        if($("#totalPriceVender").val()=="0"|| typeof $("#totalPriceVender").val() =='undefined'){
            ...
            var param = addOverseaMergeParam();
             jQuery.ajax({
                 type : "POST",
                 dataType : "json",
                 url : actionUrl,
    ...
    function flushCross(type){
        ...
        if(type=="1"){
            ...
            if(!isEmpty($(".crossSkuVender").html())) {
     ...
     * @param _id
     */
    function delete_Invoice(id, type, callback) {
      $('#mainId').hide();
    ...
    function useOrCancelCoupon(url, id, obj, flag, couponType) {
      ...
      jQuery.ajax({
        ...
        success : function(result) {
          ...
          checkPaymentPasswordComb(JING_PWD_TYPE,0);
     ...
     * @param type
     */
    function checkPaymentPasswordComb(type, giftCardType) {
        var url = OrderAppConfig.DynamicDomain + "/order/checkPaymentPasswordComb.action";
        var param = addFlowTypeParam();
        param = addOverseaMergeParam(param);
        jQuery.ajax({
            type : "POST",
    ...
    }window.checkPaymentPasswordComb = checkPaymentPasswordComb;
     ...
     * @param type
     */
    function monitorFunction(funcName) {
        try {
            var url = OrderAppConfig.DynamicDomain + "/order/monitorFunction.action?funcName=" + funcName;
    ...
    }window.monitorFunction = monitorFunction;
     ...
     * @param type
     */
    function checkPaymentPasswordNew(type, giftCardType) {
        var result = {
            "fundsPwd": $("#isFundsPwd").val() == 'true',
            "need": $("#isNeed").val() == 'true',
            "shortPwdActive": $("#isShortPwdActive").val() == 'true',
            "longPwdActive": $("#isLongPwdActive").val() == 'true'
        }
        if (!result.fundsPwd) {
            // 增加余额提示开启密码的显示
            $("#safeBalancePart").removeClass("hide");
        ...
        // 显示OR隐藏密码框
        needPaymentPasswordNew(result);
    }window.checkPaymentPasswordNew = checkPaymentPasswordNew;
                ...
                $("#tooltip-box06 b.free-freight").each(function() {
                    ...
                    if (prop != 0) { // pop
                        if (freightSop == "0.00" || freightSop == "0" || freightSop == "") { // 原有判断逻辑
                            ...
                            if(orderPrice.sopJzdFreight !=null && orderPrice.sopJzdFreight[prop] !=null){
                            	var popJzd0 = orderPrice.sopJzdFreight[prop][101];
                            	var popJzd1 = orderPrice.sopJzdFreight[prop][102];
                            	var popJzd2 = orderPrice.sopJzdFreight[prop][121];
                            	var popJzd3 = orderPrice.sopJzdFreight[prop][122];
                            	freightSop = freightSop - toNumber(popJzd0) - toNumber(popJzd1)- toNumber(popJzd2) - toNumber(popJzd3);
                            }
                            // 展示基础运费
    ...
    function flushOrderPrice(orderPrice, isFlushSkuList,typeFlag) {
      ...
      //修改快递到车服务费信息
      if (orderPrice.carDeliveryServiceTotalFee != null && orderPrice.carDeliveryServiceTotalFee > 0) {
          $("#carDeliveryServiceTotalFee").text("￥" + orderPrice.carDeliveryServiceTotalFee.toFixed(2));
          $("#showCarDeliveryServiceTotalFee").css("display", "block");
        } else {
          $("#showCarDeliveryServiceTotalFee").css("display", "none");
        }
      ...
      if(gstTaxAmount>0){
          ...
          if($("#downGstTaxAmount").val() == "1"){
    		  $("#sumYfwkzj_sf").html("应付尾款总计（含运费、税费）:");
    		  $("#sumQkhj_sf").html("全款合计（含运费、税费）：");
    	  }
      }else{
          $("#shuifeiId").addClass("hide");
          if($("#downGstTaxAmount").val() == "1"){
    		  $("#sumYfwkzj_sf").html("应付尾款总计:");
    		  $("#sumQkhj_sf").html("全款合计：");
    	  }
      }
    ...
    function flushOrderPriceByCoupon() {
        ...
        if ($("#hiddenFreight_coupon")[0]) {
            if(+$("#hiddenFreight_coupon").val() > 0) {
                ...
                $("#freightPriceId").html("<font color='#000000'> ￥" + $("#hiddenFreight_coupon").val() + "</font>");
            }
        }
      ...
      //修改领货码
        ...
        //修改税费金额
        if($("#couponGstTaxAmount").val()){
        	$("#gstTaxAmountId").html("<i class='freight-icon gstTaxAmount'></i><font color='#FF6600'> ￥"+Number($("#couponGstTaxAmount").val()).toFixed(2)+"</font>");
        }
    ...
    /**
     * 实体优惠券输入事件 对实体券输入没有生效，事件绑定错误。没有线上bug提出来，所以是否需要修改，等上级指示。 DYY
     */
     ...
     * 加载页面异步相关信息
     */
    function loadOrderExtNew(vendorRemarks) {
         // ===============3.备注==============
         var vendorRemarkMap = asyncCurrentOrderResultView.vendorRemarkMap;
         for(var key in vendorRemarkMap){
             if(vendorRemarkMap[key].show){
         ...
         // ==================5.加载余额==================
         var extBalance = asyncCurrentOrderResultView.balance;
         if (extBalance && extBalance.success && extBalance.checked) {
             $('#balanceitem').removeClass('hide');
             if(!$('#selectOrderBalance').prop("checked")){
                 $('#selectOrderBalance').click();
             }
         }else{
             if(extBalance.leavyMoney > 0 && $('#virtual_balance_tmp').length > 0){
                 try {
                     $('#balanceitem').removeClass('hide');
                     var html = template('virtual_balance_tmp', {'balance':extBalance.leavyMoney,'checked':extBalance.checked} );
                     $("#jdBalance").html(html);
                 }catch(err) {}
             }
         }
         // ==================6.加载礼品==================
         var extGift = {
             "availableGiftNum": asyncCurrentOrderResultView.availableGiftNum,
             "availableConsignmentGiftNum": asyncCurrentOrderResultView.availableConsignmentGiftNum,
             "totalGiftNum": asyncCurrentOrderResultView.totalGiftNum,
             "totalConsignmentGiftNum": asyncCurrentOrderResultView.totalConsignmentGiftNum
         };
         if(window.virtual){window.virtual.setAvailableNumbers(extGift);}
         // 京豆优惠购是否足额和京豆优惠购商品id
         var showOpenPayPwd = asyncCurrentOrderResultView.showOpenPayPwd;
         var existsJdbeanPromotion = asyncCurrentOrderResultView.existsJdbeanPromotion;
         var checkJdbeanPromotion = asyncCurrentOrderResultView.checkJdbeanPromotion;
         var shortPwdActive = $("#isShortPwdActive").val() == 'true';
         // 成功后如果有divID直接放入div，没有则返回结果
         checkShowOpenPwd(showOpenPayPwd, existsJdbeanPromotion, checkJdbeanPromotion, shortPwdActive);
         //优惠券异步加载可用优惠券数量
         if($("#isNewVertual").val() == "true"){
             vertualCoupontips();
             refushHasAvailableVertual(asyncCurrentOrderResultView);
         }else if($("#invokeNewCouponInterface").val() == "true"){
             showCouponAvailableNumByRpc(asyncCurrentOrderResultView.availableCouponNum);
             showCouponSkuList();
         }
    }
    ...
    function loadOrderExt(vendorRemarks) {
        if( $("#downDybZffs").val() == "1") {
            monitorFunction("loadOrderExt");
        }
      ...
      jQuery.ajax({
        ...
        success : function(dataResult, textStatus) {
          ...
          if (dataResult.balance && dataResult.balance.success && dataResult.balance.checked) {
              ...
              if(dataResult.balance.leavyMoney > 0 && $('#virtual_balance_tmp').length > 0){
                  try {
                      $('#balanceitem').removeClass('hide');
          ...
          if($("#yshbQk").val() == "1" && $("#flowType").val() == "15" && ($("#presaleStepPay").val()=="2" || $("#presaleStepPay").val()=="4")){
        	  if(dataResult.redPacketResultVO.redPacketTotalBalance == 0){
            	  $("#red-packet").hide();
            	  $(".redpack-main.ui-switchable-panel").hide();
              }else{
            	  $("#red-packet").show();
            	  $(".redpack-main.ui-switchable-panel").show();
              }
              if($("#EarnestPayRadio").hasClass("item-selected")){
            	  $(".step-cont.order-presale").show();
            	  $(".step-cont.order-virtual").hide();
              }
              if($("#AllPayRadio").hasClass("item-selected")){
            	  $(".step-cont.order-presale").hide();
            	  $(".step-cont.order-virtual").show();
            	  vertualShow();
              }
              if($("#AllPayRadio").hasClass("item-selected")){
              ...
              if($("#AllPayRadio").hasClass("item-selected") && dataResult.redPacketResultVO.redPacketTotalUsableBalance>0){
            	  $('#red-packet-price').show();
              }else{
            	  $('#red-packet-price').hide();
              }
          }
        },
        error : function(XMLHttpResponse) {
     ...
     * 打开国际物流综合服务协议
     */
    function openGjwlzhfwxyAgree () {
        var hkbugTip = $(document.body).dialog({
            title: $('#sumGjwlzhfwxy').val(),
            width: 940,
            height:500,
            fixed: true,
            maskHas: false,
            source: $('#overseabuy-box02').html() + '',
            onReady: function(){
                $('#overseabuy-box02-btn .btn-1').click(function(event) {
                    $("#overseamtbuy").attr("checked",true);
                    hkbugTip.close();
                });
            }
        ...
        return false;
    }window.openGjwlzhfwxyAgree=openGjwlzhfwxyAgree;
    /**
     * 打开合并支付协议
     */
    function openOverseaMergeAgree () {
        var hbzfTip = $(document.body).dialog({
            title: '用户购买须知',
            width: 940,
            height:420,
            fixed: true,
            maskHas: false,
            source: $('#overseaMergebuy-box01').html() + '',
            onReady: function(){
                $('#overseaMergebuy-box01-btn .btn-1').click(function(event) {
                    $("#overseaMerge_check").attr("checked",true);
                    hbzfTip.close();
                });
            }
        });
        return false;
    }window.openOverseaMergeAgree=openOverseaMergeAgree;
    ...
    function submit_Order(enterPriseUser,agreement,jxj) {
        ...
        // 合并支付协议勾选
        var hasOverseaMerge=$("#hasOverseaMerge").val();
        var isInternational=$("#isInternational").val();
        if(hasOverseaMerge == 'true' || isInternational  == 'true') {
            if(!$("#overseaMerge_check").is(":checked")) {
                alert("请查阅并同意协议后继续下单");
                return;
            }
        }
        ...
        if(canSubmitFlag){
            ...
            actionUrl = addOverseaMergeParam(actionUrl);
          ...
          param +="&fq="+fq;
              ...
              jQuery.ajax({
                ...
                success : function(result) {
                  ...
                  if (result.success) {
                      // 企业主子账号-提交审核后跳转到订单中心
                      var accountVerify = $('#accountVerify').val();
                    // 企业用户合并支付
                    ...
                    if (result.goJumpOrderCenter) {
                      ...
                      if(result.overseaMerge) {
                          successUrl = successUrl +"&overseaMerge=1";
                      }
                        if(result.international) {
                            successUrl = successUrl +"&isInternational=1";
                        }
                      if(result.orderType){
            ...
            if (result.message != null) {
              ...
              if(result.resultCode == 600160){
                ...
                $("#submit_loading").remove();
                showSubmitErrorMessage("检测到地址中的所在地区填写不完整，请重新填写完整的地址所在信息！");
    ...
    function showScaleBuyDialog(result){
         ...
         var content = '';
            for(var i = 0; i < skuArray.length; i++){
                ...
                '<div class="p-name" style="height: auto;">'+name+
    ...
    */
    ...
    function useOldRepalceNew() {
      ...
      var param = addFlowTypeParam("isReplace=" + isReplace);
        param = addOverseaMergeParam(param);
        //NOTE: ddd
      jQuery.ajax({
        type : "POST",
     ...
     * 商品换区
     */
    function productAreaChange() {
        var actionUrl = OrderAppConfig.AsyncDomain + "/productAreaChange.action";
        var param = addFlowTypeParam();
        param = addOverseaMergeParam(param);
        jQuery.ajax({
            type : "POST",
            ...
            data : param,
            cache : false,
            success : function(dataResult, testStatus) {
                if (dataResult && dataResult.message) {
                    showProductAreaChanged(dataResult.message)
                }
            }
        });
    }
     ...
     * 换区弹出框
     * */
    function showProductAreaChanged(skuJson) {
        var skuList = jQuery.parseJSON(skuJson);
        var imgRoot = "//img10.360buyimg.com/cms/s80x80_";
        var dialogDiv = ""
        dialogDiv += "<div class='psam-thickbox'>";
        ...
        dialogDiv += "		<span class='warn-icon m-icon'></span>";
        dialogDiv += "		<div class='item-fore'>";
        dialogDiv += "			<h3>根据您当前的收货地址已经自动更换了相应的地区价格及配送方，请您知晓！</h3>";
        dialogDiv += "		</div>";
        dialogDiv += "	</div>";
        dialogDiv += "	<div class='goods-items'>";
        for(var i = 0; i < skuList.length; i ++) {
        ...
        dialogDiv += "	</div>";
        dialogDiv += "	<div class='op-btns ac'>";
        dialogDiv += "		<a href='javascript:void(0)' class='btn-1' onclick='closeBigItemDialogDiv()'>知道了</a>";
        dialogDiv += "	</div>";
        dialogDiv += "</div>";
    ...
    }window.showProductAreaChanged=showProductAreaChanged;

    function closeBigItemDialogDiv() {
        $.closeDialog();
        bigItemGoOrder();
    }window.closeBigItemDialogDiv=closeBigItemDialogDiv;
    ...
    function showOrHideJdBean(types) {
      if ($("#orderBeanItem").hasClass("toggle-active")) {
        ...
        var params = addFlowTypeParam();
          params = addOverseaMergeParam(params);
        jQuery.ajax({
          type : "POST",
          dataType : "html",
          data : params,
          ...
          success : function(result) {
            ...
            $("#jdBeans-new").html(result);
              // wangjianwen 添加判断，当使用新版本时(newVersionTrade == 1)，isFlushSkuList传false，防止调用save_pay()方法；
    ...
    function cancelJdBeanWithoutOpen() {
      ...
      jQuery.ajax({
        ...
        success : function(result) {
            // wangjianwen 添加判断，当使用新版本时(newVersionTrade == 1)，isFlushSkuList传false，防止调用save_pay()方法；
            flushOrderPrice(getJdBeanOrderPrice(), false,1);
            if(getCheckFundsPwdResult() != undefined && getCheckFundsPwdResult() != null) {
                checkPaymentPasswordNew("JdBean");
            } else {
                checkPaymentPasswordComb("JdBean");
            }
        },
        error : function(XMLHttpResponse) {
        }
    ...
    function addOverseaMergeParam(params) {
        var overseaMerge = $("#overseaMerge").val();
        //增加预售现货买参数
        var presaleStockSign = $("#presaleStockSign").val();
        if(overseaMerge == "true"){
    		if (isEmpty(params)) {
    			params =  "overseaMerge=" + 1;
            } else {
            	params = params + "&overseaMerge=" + 1;
            }
    	}
    	if(presaleStockSign == "true"){
    		if (isEmpty(params)) {
    			params =  "presaleStockSign=" + 1;
            } else {
            	params =  params + "&presaleStockSign=" + 1;
            }
    	}
    	return params;

    }window.addOverseaMergeParam=addOverseaMergeParam;
    ...
    if (isUnregister || isUnregister == "true") {
      ...
      //isNewUser(); // 新用户展开地址
        if($("#downDyb").val() == "1" && $("#flowType").val() != "15") {
            loadOrderExtNew();
        } else {
            loadOrderExt();
        }
                ...
                if ($(this).attr("id") == venderId) {
                    ...
                    if (2 == venderInsurance.type) {
                        if (venderInsurance.showFlag) {
                            ...
                            $(this).find("input").attr("uuid", venderInsurance.uuid);
                            // 选中的情况
      ...
      if ($("#presaleMobile input").size() > 0) {// 点击保存
        ...
        if (testMobile(mobile) && mobile.indexOf("*") < 0 ) {
          ...
          $("#cancelOperatePresaleMob").hide();
            ...
            var html = "<input type=\"text\" class=\"itxt error-itxt\" maxlength=\"" +maxLength+ "\"><span class=\"error-msg\" style=\"color:red\">请输入正确的手机号</span></span>";
            $("#presaleMobile").html(html);
        }
      } else {// 点击修改
          $("#presaleMobile").html("<input type=\"text\"  class=\"itxt focus-itxt\" maxlength=\"" +maxLength+ "\" />");
    ...
    function testMobile(mobile) {
        if(isOverseaAddressForMobile()) {
            return check_mobile_newCheck(mobile);
        }
        return check_mobile_new(mobile);
    }
    function isOverseaAddressForMobile() {
        // 港澳台、海外
        var hongKongId = $("#hongKongId").val();
        var taiWanId = $("#taiWanId").val();
        var overSeasId = $("#overSeasId").val();
        ...
        if(consignee_area!=null) {
            var consignee_provinceId = consignee_area.split("-")[0];
            if($("#presaleMobileCheck").val()=="1" && (consignee_provinceId == hongKongId || consignee_provinceId == overSeasId || consignee_provinceId == taiWanId)){
                return true;
            }
        }
        return false;
    ...
    function doGetVendorName(){
      ...
      if(flowType == "1" || flowType == "13"){
        ...
        var samVenderList = getSamVenderList();
        var walmartVenderList = getWalmartVenderList();
        if(!samVenderList || !walmartVenderList) {
            return;
        }
        $(".vendor_name_h").each(function() {
            if($(this).attr("fitVenderName") == 'true' ) {
                // 特殊流程，特殊店铺名称(分堆)
            } else {
                ...
                if(key == "0"){
                    ...
                    var venderName = $(this).attr("venderName")
                    if (venderName != null && venderName != "undefined") {
                        if (venderName.length > 16)
                            venderName = venderName.substring(0, 15) + "";
                        if(!$.isEmptyObject(samVenderList) && $.inArray(key, samVenderList)  != -1){//山姆店显示标示
                            $(this).html("商家："+'<em class="project-sam-icon project-sam-icon-b mr5"></em>' + venderName);
                        } else if (!$.isEmptyObject(walmartVenderList) && $.inArray(key, walmartVenderList)  != -1) {//沃尔玛店显示标识
                            $(this).html("商家："+'<em class="project-walmart-icon mr5"></em>' + venderName);
                        } else {
                            $(this).html("商家："+ venderName);
                        }
                    }
                }
            }
        ...
        $(".vendor_name_freight").each(function() {
            if($(this).attr("fitVenderName") == 'true' ) {
                ...
                if(key == "0"){
                    ...
                    var venderName = $(this).attr("venderName");
                    var venderId = $(this).attr("id");
                    // 兼容处理，等VenderFreightDetailMap结构中有免运费的商家名称后，在替换，先用店铺列表的商家名称
                    if(isEmpty(venderName)) {
                        venderName = $('.vendor_name_h.vendor_name_' + venderId).attr('vendername');
                    }
                    if (venderName != null && venderName != "undefined") {
                        if (venderName.length > 16)
                            venderName = venderName.substring(0, 15) + "...";
        ...
        $(".vendor_name_yanbao").each(function() {
            ...
            if(key == 0 || key == "0"){
              ...
              // 等 balanceApi com.jd.core.server.balance.order.vo.SkuVO 添加venderName再改造。
          }
        });
    ...
    function openEditConsigneeDialog(id){
      var url = OrderAppConfig.DynamicDomain + "/consignee/editConsignee.action?consigneeId="+id;
        if($("#consignee_index_div_"+id).hasClass("consignee-item-disable")){
            url = url+"&oldAddr=1"
        }
      url = addFlowTypeParam(url);
        url= addOverseaMergeParam(url);
    ...
    var g_outSkus;
    var g_noStockType;
    function doOutSku(outSkus, noStockType,address, cartEmpty){
        // js兼容，20201102更新，上线后可删除
        if(typeof cartEmpty === 'undefined') {
            if(noStockType == "600178") {
                cartEmpty = true;
            } else if (noStockType == "600157" || noStockType == "600182" || noStockType == "0" || noStockType == "600178") {
                cartEmpty = false;
            }
        }
      g_outSkus = outSkus;
    ...
    function fillSkuList(outSkuArr,ckArray,address){
      ...
      var goodsHtmls = "";
        ...
        if (outSkuArr != null) {
            var outSkuString = outSkuArr.join(",");
            goodsHtmls += "<input type='hidden' id='noStockSkus' value='" + outSkuString + "'/>";
        }
        for(var i = 0; i < allGoodsObj.length; i++){
            var goodsItem = allGoodsObj[i];
    ...
    function cloneProducts(goodsEles){
      ...
      for(var i = 0; i < goodsEles.length; i++){
        ...
        for(var j = 0; j < gifts.length; j++){
          ...
          if(isEmpty(skuInfo)) {
              skuInfo = gift.find("p").first().html();
          }
          var giftType = gift.attr("gift-type");
            var noStock = gift.attr("gift-stock") == "false";
          giftArr.push({"skuId" :  skuId, "skuInfo" : skuInfo,"giftType":giftType,"noStock":noStock});
        }
    ...
    */
    ...
    function selectBuyerFreightInsurance(thisElement, venderId) {
        ...
        //uuid
        var uuid =$(thisElement).attr('uuid') || '';
        // 操作之前状态
        var preStatus = !postStatus;
        //服务费类型
        var serviceFeeType = "1";
    
        // 准备参数
        var param = "venderId=" + venderId + "&checkedFlag=" + postStatus+ "&uuid=" + uuid + "&serviceFeeType=" + serviceFeeType;
        param = addFlowTypeParam(param);
        param = addOverseaMergeParam(param);
     ...
     * @author wxw
     */
    function selectXunHuanBZFreightInsurance(thisElement, venderId) {
        var FID = "";
        if($(".selfPickInCommonItem").hasClass("item-selected")){
            FID = "_ZT";
        }
        // 选择器
        var selector = "#packaging_services_check"+FID;
        // 操作之后状态
        var postStatus = $(thisElement).prop("checked");
        //uuid
        var uuid =$(thisElement).attr('uuid') || '';
     ...
     * @author wxw
     */
    function selectJingZundaFreightInsurance(thisElement, venderId,postStatus) {
        // 选择器
        var selector = ".buyer_freight_insurance#vender_" + venderId;
        // 操作之后状态
    ...
    function save_belong_area(id){
        ...
        var param = "consigneeParam.newId=" + consignee_id + "&consigneeParam.type=" + consignee_type
                    ...
                    if($('#allHaoKa').val()=="true"){
                        ...
                        if(consigneeResult.resetCommon){
                            if(consigneeResult.selectNormalInvoiceContent=="不开发票"){
                                ...
                                //电子增值税发票
                                parent.$("#part-inv").html("<span class='mr10'>电子增值税发票&nbsp; </span><span class='mr10'> "+consigneeResult.selectInvoiceTitle+"&nbsp; </span><span class='mr10'>"+consigneeResult.selectNormalInvoiceContent+"&nbsp; </span><a onclick='edit_Invoice()' class='ftx-05 invoice-edit' href='#none'>修改</a>");
                            }
                        }
                    }
    ...
    //					reloadCoupon(consigneeResult.reloadCoupon);
                    ...
                    $("#saveConsigneeTitleDiv").parent().attr("onclick", "save_Consignee()").removeAttr("style");
                    if (consigneeResult.message != null && consigneeResult.message != ""){
                        // showMessageWarn(consigneeResult.message);
                        $("#addressBackCheckMessage").addClass("message");
                        $("#addressBackCheckMessage").html(consigneeResult.message);
                        return;
                    }
                }
                //非新用户
    ...
    /*限购 卡单提示*/
    function showPurchaseList(items,errorMessage,newMessage){
        var confinel='';
        ...
        for( var  k = 0 ;k<items.length ; k++ ){
            itemObjs.each(function(i,itemObj){
                if(items[k].skuId === itemObj.getAttribute('goods-id')){
                    ...
                    if(typeof newMessage !=='undefined' && newMessage != null && newMessage !=""){
                    	item_error = newMessage;
                    }
